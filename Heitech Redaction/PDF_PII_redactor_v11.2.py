...
if page_mode == "Upload PDF":
    ...
    if st.session_state.get("processing_complete", False) and "widgets_df" in st.session_state:
        ...
        # Add the download button for the redacted PDF
        if "output_buffer" in st.session_state:
            st.download_button(
                "⬇️ Download Redacted PDF",
                data=st.session_state["output_buffer"].getvalue(),
...
if page_mode == "Edit":
    ...
    # Debugging: Log the current state of editor_tags
    st.write("Current editor_tags:", st.session_state["editor_tags"])
        ...
        if st.session_state["selected_tag_type"]:
            ...
            for idx, row in widgets_df.iterrows():
                if row['Field Value'].strip():
                    ...
                    # Ensure editor_tags is initialized and contains data before accessing
                    if "editor_tags" in st.session_state and isinstance(st.session_state["editor_tags"], list):
                        field_tags = [tag for tag in st.session_state["editor_tags"] if tag["field_idx"] == idx]
                    else:
                        field_tags = []  # Default to an empty list if not initialized
                    if field_tags:
