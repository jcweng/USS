...
if page_mode == "📥 Upload PDF":
    ...
    if st.button("✅ Confirm and Run Correction"):
        corrected_text = sample_text  # default fallback

        if spell_level == "1":
            from spellchecker import SpellChecker
            spell = SpellChecker()
            corrected_text = " ".join([spell.correction(w) or w for w in sample_text.split()])
            ...
            import openai
            openai.api_key = st.secrets.get("OPENAI_API_KEY", "sk-YOUR-KEY")  # adjust key handling
            prompt = f"Correct any spelling and grammar errors in this text:\n\n{sample_text}\n\nCorrected version:"
