...
def apply_text_corrections(text, spell_level, grammar_level, fluency_level):
    ...
    if spell_level == "2":
        from symspellpy.symspellpy import SymSpell, Verbosity          # ← Load SymSpell classes
        sym_spell = SymSpell(max_dictionary_edit_distance=2, 
                            prefix_length=7)                          # ← Create instance with params
        dictionary_path = os.path.join(os.path.dirname(__file__), "frequency_dictionary_en_82_765.txt")

        dictionary_path = os.path.join(os.path.dirname(__file__), 
                                    "frequency_dictionary_en_82_765.txt")
        if not sym_spell.load_dictionary(dictionary_path, term_index=0, count_index=1):
            st.warning("SymSpell dictionary failed to load.")
        else:
            corrected_tokens = []
            for word in corrected_text.split():
                suggestions = sym_spell.lookup(word, Verbosity.CLOSEST, max_edit_distance=2)
                if suggestions:
                    corrected_tokens.append(suggestions[0].term)       # ← Take top suggestion
                else:
                    corrected_tokens.append(word)
            corrected_text = " ".join(corrected_tokens)                # ← Join all corrected words
    if fluency_level == "1":
...
if page_mode == "📥 Upload PDF":
    ...
    with col2:
        grammar_level = st.selectbox("Grammar Correction", options=["disable", "1", "3"])
    ...
    def correct_text_preview(sample_text, spell_level, grammar_level, fluency_level):
        return apply_text_corrections(sample_text, spell_level, grammar_level, fluency_level)

    if st.button("✅ Run Preview on Sample Text"):
        preview_output = correct_text_preview(sample_text, spell_level, grammar_level, fluency_level)
