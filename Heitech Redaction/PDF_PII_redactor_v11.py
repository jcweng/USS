...
st.title("FDA 3500A MedWatch Form Redaction System")
...
if page_mode == "Verify":
    st.title("Verify Initial Redactions")
        ...
        if st.session_state["selected_tag_type"]:
            ...
            for idx, row in widgets_df.iterrows():
                if row['Field Value'].strip():
                    ...
                    st.markdown(f"<h4 style='text-align: left; margin-bottom: 2px; margin-top: 10px;'>{row['Display Name']} (Page {row['Page']})</h4>", unsafe_allow_html=True)
